...
func solve(d int, gamers []gamer) (t, x, y float64) {
	...
	const scale = 11 // увеличиваем разрешение на каждой итерации
	...
	for {
		...
		for _, p := range unaval {
			x1, y1 := p.x-step, p.y-step
			x2, y2 := p.x+step, p.y+step
		...
		t1, t2 = max(0, t1-step*4), min(2000, t2+step*4) // минимальная скорость 1
		if debugEnable {
