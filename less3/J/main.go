...
type net struct {
	...
	totalPartsCount int
	partOrder       []int
	partSrc         []int // хост для запроса каждой части
	roundCount      int
}
...
func (nt *net) fillFirstHost() {
	...
	for partID := 0; partID < nt.k; partID++ {
		h.receivePartFrom(0, partID)
	}

	// в порядке бреда: части не скачены, а появились сами собой (как подкустовные выползни)
	// h.partsCount = 0
}
...
func (nt *net) doRound() {
	...
	for partID := 0; partID < nt.k; partID++ {
		...
		for id := 1; id < nt.n; id++ {
			h := &nt.host[id]
			if h.partExists[partID] && h.partsCount < minimum {
				hostID = id
				minimum = h.partsCount
			}
		}

		nt.partSrc[partID] = hostID
	}
...
func (nt *net) hostReceivedPart(hostID, partID int) {
...
func (h *host) doRequest() {
	...
	// Каждое устройство выбирает отсутствующую на нем часть...
	// После этого устройство делает запрос выбранной части обновления у одного из устройств...

	// выбираем первую отсутствующую часть в порядке предпочтения
...
func (h *host) receiveRequestFrom(fromID, partID int) {
...
func (h *host) receivePartFrom(fromID, partID int) {
	...
	if h.partsCount == h.net.k {
		h.finish = h.net.roundCount
	}
...
func solve(n, k int) []int {
	...
	for id := 1; id < len(nt.host); id++ {
		res = append(res, nt.host[id].finish)
	}
